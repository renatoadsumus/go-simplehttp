...
export TASK_VERSION=$(aws ecs register-task-definition --region us-east-1 --cli-input-json file://task-definition.json | jq --raw-output '.taskDefinition.revision')
...
then	
	sed -i 's/TD_VERSION/'$TASK_VERSION'/' service-definition.json
	sleep 5
	aws ecs create-service --region us-east-1 --cli-input-json file://service-definition.json
